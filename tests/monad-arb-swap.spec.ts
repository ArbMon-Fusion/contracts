...
import { ChainConfig, config } from './config-monad-arb'
...
const userPk = process.env.USER_PRIVATE_KEY!.toString() // needs monad 
const resolverPk = process.env.DEPLOYER_PRIVATE_KEY!.toString() // needs eth
const WETH = "0x980B62Da83eFf3D4576C647993b0c1D7faf17c73" // Arbitrum Sepolia WETH
const WMON = "0x760AfE86e5de5fa0Ee542fc7B7B713e1c5425701" // Monad Testnet WMON

describe('Arbitrum Sepolia to Monad Testnet Swap', () => {
    const srcChainId = config.chain.source.chainId      // 10143
    const dstChainId = config.chain.destination.chainId // 421614
    ...
    beforeAll(async () => {
        console.log('🚀 Setting up Monad Testnet -> Arbitrum Sepolia test...')
        ...
        if (config.chain.source.createFork) {
            console.log('💰 Funding user with WMON on fork...')
            // Impersonate WMON rich account to transfer WMON to user
            await src.provider.send('anvil_impersonateAccount', ['0xFA735CcA8424e4eF30980653bf9015331d9929dB'])
            const wmonRichAccount = await src.provider.getSigner('0xFA735CcA8424e4eF30980653bf9015331d9929dB')
            ...
            console.log(`User balance before getting 10 wmon: ${await srcChainUser.tokenBalance(WMON)}`)
            const wmonRichAddress = await wmonRichAccount.getAddress()
            console.log(`Wmonrich account token balance before giving 10 wmon: ${await wmonContract.balanceOf(wmonRichAddress)}`)


            await srcChainUser.topUpFromDonor(
                config.chain.source.tokens.WMON.address,
                config.chain.source.tokens.WMON.donor,
                parseUnits('10', 18)
            )
            ...
            console.log('✅ User funded with WMON on Monad Testnet fork')

            // Impersonate a rich account to fund test accounts
            await src.provider.send('anvil_impersonateAccount', ['0x760afe86e5de5fa0ee542fc7b7b713e1c5425701'])
            const richAccount1 = await src.provider.getSigner('0x760afe86e5de5fa0ee542fc7b7b713e1c5425701')
            ...
            console.log('✅ User funded with WMON and MON on Monad Testnet fork')
            console.log(`User balance after getting 10 wmon: ${await srcChainUser.tokenBalance(WMON)}`)
        ...
        if (config.chain.destination.createFork) {
            console.log('💰 Funding resolver with WETH on fork...')

            // Impersonate WETH rich account to transfer WETH to resolver
            await dst.provider.send('anvil_impersonateAccount', ['0xd7512902999b34af2B2940Eb8827CC8345DC77C6'])
            const wethRichAccount = await dst.provider.getSigner('0xd7512902999b34af2B2940Eb8827CC8345DC77C6')
            ...
            const wethTransferTx = await wethContract.transfer(await dstChainResolver.getAddress(), parseEther('10'))
            await wethTransferTx.wait()

            // Impersonate a rich account to fund test accounts
            await dst.provider.send('anvil_impersonateAccount', ['0x980B62Da83eFf3D4576C647993b0c1D7faf17c73'])
            const richAccount2 = await dst.provider.getSigner('0x980B62Da83eFf3D4576C647993b0c1D7faf17c73')
            ...
            await dstResolverContract.topUpFromDonor(
                config.chain.destination.tokens.WETH.address,
                config.chain.destination.tokens.WETH.donor,
                parseUnits('20', 18)
            )

            await dstChainResolver.transfer(dst.resolver, parseEther('1'))
            await dstResolverContract.unlimitedApprove(config.chain.destination.tokens.WETH.address, dst.escrowFactory)
        }
    ...
    describe('WMON → WETH Cross-Chain Swap', () => {
        it('should swap WMON (Monad) → WETH (Arbitrum)', async () => {
            console.log('\n🔄 Starting WMON → WETH cross-chain swap...')
            ...
            console.log("resolver initial WETH token balance:", await wethContract.balanceOf(await dstChainResolver.getAddress()))
            ...
            console.log("srcchain resolver balance:", await wmonContract.balanceOf(await srcChainResolver.getAddress()))
            ...
            console.log(`🔄 Creating destination escrow on Arbitrum Sepolia...`)
            const { txHash: dstDepositHash, blockTimestamp: dstDeployedAt } = await dstChainResolver.send(
                resolverContract.deployDst(dstImmutables)
            )
            console.log(`✅ Destination escrow created on Arbitrum Sepolia: ${dstDepositHash}`)
            ...
            console.log(`💸 User withdrawing WETH from Arbitrum Sepolia escrow: ${dstEscrowAddress}`)
            ...
            console.log(`💸 Resolver withdrawing WMON from Arbitrum escrow: ${srcEscrowAddress}`)
            ...
            expect(dstBalanceChange).toBeGreaterThan(parseEther('0.009')) // Received close to 0.01 WETH

            // User should have spent WMON on source
            expect(srcBalanceChange).toBeGreaterThanOrEqual(swapAmount)

            console.log('🎉 WMON -> WETH cross-chain swap completed successfully!')
